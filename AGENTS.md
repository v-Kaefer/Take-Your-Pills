...
Runtime pacing numbers (speed curve, boost durations, score accumulation
constants, etc.) live in one place: the `Balance` autoload
...
Collectable `score_value` entries in `.tscn` files and chunk spawn/recycle
buffers are still scene/controller-owned data unless a task explicitly moves
their runtime reads into `Balance.config`.
...
- [ ] After 8 seconds (base boost duration, see `Balance.config.speed_up_boost_duration`): speed returns to normal, stripes reset to grey
