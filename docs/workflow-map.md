...
| `.githooks/pre-push` | `infra` | Local Git hook | Path-gated checks before push. Runs only the checks that match the files changed in the push. | Git upstream detection, syntax/lint checks | Local-only |
