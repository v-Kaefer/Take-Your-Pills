...
| `.github/workflows/pr-hygiene.yml` | `infra` | `pull_request_target` on PR lifecycle events | Sync PR labels, milestone, assignees, linked task relationship, and Project status from the linked task. | `governance_bootstrap.pr_hygiene`, `TAKE_YOUR_PILLS_PAT`, `GOVERNANCE_PROJECT_NUMBER` | Active |
...
| `.github/workflows/governance-bootstrap.yml` | `infra` | `workflow_dispatch` | Manual sync of labels, milestones, project setup, and issue generation. | `governance_bootstrap.cli`, `TAKE_YOUR_PILLS_PAT` | Active |
