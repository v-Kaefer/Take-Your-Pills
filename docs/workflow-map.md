...
| `.github/workflows/pr-hygiene.yml` | `infra` | `workflow_run` after `PR guardrails` succeeds, plus direct `pull_request_target` on draft/close lifecycle events | Sync PR labels, milestone, assignees, linked task relationship, and Project status from the linked task after the PR body has been normalized. Hotfix PRs are skipped. | `governance_bootstrap.pr_hygiene`, live PR reads from GitHub, `github.token`, optional `GOVERNANCE_PAT` for Project v2, `GOVERNANCE_PROJECT_NUMBER` | Active |
...
| `.github/workflows/game-tests.yml` | `godot` | `workflow_run` after `Godot smoke check` succeeds, plus `workflow_dispatch` | Refined Godot gameplay suite for player, obstacle, chunk, and game-flow behavior, with a live PR file check so it skips cleanly when gameplay paths did not change. | `tests/godot/*`, PR file list API, `godot-gdunit-labs/gdUnit4-action@v1.3.1` | Active |
...
- `PR guardrails -> PR hygiene` is the governance execution order. `PR hygiene` now runs from `workflow_run` after guardrails succeeds so it consumes the updated PR body instead of stale template content.
- `Godot smoke check -> Game tests` is the runtime execution order. Smoke stays cheap and immediate; gdUnit runs only after smoke passes and the associated PR actually touches gameplay paths.
- PR-facing workflows cancel stale PR runs with concurrency to avoid duplicate comments and wasted validation on superseded commits.
